...
function cleanSave(raw){
 ...
 if(Array.isArray(raw.stars)){out.stars=[0,0,0];for(var z=0;z<3;z++)if(Number.isFinite(raw.stars[z]))out.stars[z]=clamp(Math.floor(raw.stars[z]),0,3);}if(["yellow","red","blue","green","metal","chrome"].indexOf(raw.paint)>=0)out.paint=raw.paint;if(typeof raw.sound==="boolean")out.sound=raw.sound;return out;
}
