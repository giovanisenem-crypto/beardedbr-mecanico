...
function cleanSave(raw){
 var out={credits:0,best:0,bestTime:null,upgrades:{engine:0,tires:0,forks:0},sound:true,paint:"yellow",horn:false,beacon:false};
 ...
 if(Array.isArray(raw.stars)){out.stars=[0,0,0];for(var z=0;z<3;z++)if(Number.isFinite(raw.stars[z]))out.stars[z]=clamp(Math.floor(raw.stars[z]),0,3);}if(["yellow","red","blue","green","metal","chrome"].indexOf(raw.paint)>=0)out.paint=raw.paint;if(typeof raw.horn==="boolean")out.horn=raw.horn;if(typeof raw.beacon==="boolean")out.beacon=raw.beacon;if(typeof raw.sound==="boolean")out.sound=raw.sound;return out;
}
