...
function overlays(which){["menu","paused","result","workshop","map"].forEach(function(id){$(id).hidden=id!==which;});}
function mapInfo(){var m=$("factory-map");if(!m)return;var names=["RECEBIMENTO","ESTOQUE","PRODUÇÃO","CNC · EM BREVE"];m.innerHTML=names.map(function(n,i){var stars=(saved.stars&&saved.stars[i])||0,locked=i>2;return '<button class="map-node '+(locked?"locked":"")+'" data-phase="'+(i+1)+'"><b>'+(i+1)+'</b><span>'+n+'</span><em>'+(locked?"🔒":("★".repeat(stars)+"☆".repeat(3-stars)))+'</em></button>';}).join("");Array.prototype.forEach.call(m.querySelectorAll(".map-node"),function(b){b.onclick=function(){if(b.classList.contains("locked"))return;selectedPhase=+b.dataset.phase;goMenu();};});}function menuInfo(){
...
$("quit").addEventListener("click",goMenu);$("result-menu").addEventListener("click",goMenu);$("workshop-button").addEventListener("click",workshop);$("map-button").addEventListener("click",function(){mapInfo();overlays("map");});$("map-back").addEventListener("click",goMenu);$("intro").addEventListener("click",function(){this.hidden=true;goMenu();});$("close-workshop").addEventListener("click",goMenu);
